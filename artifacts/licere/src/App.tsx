...
import {
  ...
  Building2, ChevronDown, ChevronRight, CircleHelp, ClipboardCheck, Download, Eye, EyeOff,
  FileText, Filter, FolderOpen, LayoutDashboard, Leaf, ListFilter, ListTodo, LogIn, Menu,
  MoreHorizontal, Plus, Search, ShieldCheck, SlidersHorizontal, X, Zap,
} from 'lucide-react';
...
function Mark({ className = '' }: { className?: string }) {
  return <span className={cn('licere-mark', className)} aria-hidden="true">
    <svg viewBox="0 0 32 32" role="img" focusable="false">
      <path d="M7 6h12a3 3 0 0 1 0 6h-6a3 3 0 0 0 0 6h12" />
    </svg>
  </span>;
}
...
function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const nav = [
    ...
    { href: '/unidades', label: 'Unidades', icon: Building2, count: 5 },
    { href: '/documentos', label: 'Documentos', icon: FolderOpen },
    { href: '/tarefas', label: 'Tarefas', icon: ListTodo, count: 6 },
  ];
  const secondaryNav = [
  ...
  return <aside className={cn('sidebar', open && 'sidebar-open')}>
    ...
    <div className="nav-secondary"><span>Controle</span>{secondaryNav.map(({ href, label, icon: NavIcon }) => <Link key={href} href={href} onClick={onClose} data-testid={`link-${label.toLowerCase()}`} className={cn('nav-item', location.startsWith(href) && 'nav-item-active')}><NavIcon size={16} strokeWidth={1.8} /><span>{label}</span></Link>)}</div>
    <div className="sidebar-note"><div><b>Prioridade da semana</b><span>3 itens pedem atenção antes de sexta.</span><Link href="/tarefas" onClick={onClose}>Abrir tarefas <ChevronRight size={13} /></Link></div></div>
...
function Topbar({ onMenu, onQuickAdd }: { onMenu: () => void; onQuickAdd: () => void }) {
  ...
  return <header className="topbar"><button className="mobile-menu" onClick={onMenu} aria-label="abrir menu"><Menu size={20} /></button><div className="top-actions"><label className="global-search"><Search size={16} /><input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && setLocation(`/licencas?busca=${encodeURIComponent(search)}`)} placeholder="Buscar em tudo" aria-label="buscar em tudo" data-testid="input-global-search" /><kbd>⌘ K</kbd></label><div className="notification-wrap"><IconButton label="notificações" onClick={() => setNotificationOpen((open) => !open)} active={notificationOpen}><Bell size={18} /></IconButton>{notificationOpen && <div className="notification-popover"><b>Sem novos alertas</b><span>Seu radar de conformidade está em dia.</span></div>}</div><button className="quick-add" onClick={onQuickAdd} data-testid="button-adicionar"><Plus size={17} />Adicionar</button></div></header>;
}
function PageHeader({ eyebrow, title, description, action, actionLabel }: { eyebrow: string; title: string; description?: string; action?: () => void; actionLabel?: string }) {
  return <div className="page-header animate-rise"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1>{description && <p className="page-description">{description}</p>}</div>{action && <button className="primary-btn" onClick={action} data-testid="button-page-action"><Plus size={17} />{actionLabel}</button>}</div>;
}
...
function Overview({ licencas, condicionantes, openDetail }: { licencas: Licenca[]; condicionantes: Condicionante[]; openDetail: (item: Licenca | Condicionante | Documento) => void }) {
  ...
  return <main className="content"><PageHeader eyebrow="12 mar 2025" title="Bom dia, Marina." /><div className="metric-grid animate-rise" style={{ animationDelay: '.06s' }}><Metric label="Índice de conformidade" value="87,4%" detail="+2,8% no mês" icon={ShieldCheck} tone="green" trend="up" /><Metric label="Licenças vigentes" value="4 de 6" detail="2 pedem atenção" icon={Leaf} tone="sand" /><Metric label="Tarefas em aberto" value="3" detail="1 vence nesta semana" icon={ClipboardCheck} tone="peach" /><Metric label="Documentos atualizados" value="92%" detail="7 no acervo" icon={FolderOpen} tone="lavender" trend="up" /></div>{notice && <div className="notice-banner animate-rise" style={{ animationDelay: '.12s' }}><div className="notice-icon"><Zap size={17} /></div><div><b>Prazo próximo</b><span>Uma tarefa do CD Joinville vence em 7 dias.</span></div><Link href="/tarefas">Abrir tarefa <ChevronRight size={15} /></Link><button onClick={() => setNotice(false)} aria-label="fechar aviso"><X size={16} /></button></div>}<div className="dashboard-grid animate-rise" style={{ animationDelay: '.18s' }}><RiskChart /><PriorityList licencas={licencas} condicionantes={condicionantes} onOpen={openDetail} /></div><div className="lower-grid animate-rise" style={{ animationDelay: '.24s' }}><div className="centers-card"><div className="card-heading"><div><p className="eyebrow">Rede operacional</p><h2>Unidades</h2></div><Link href="/unidades" className="text-link">Ver unidades <ChevronRight size={14} /></Link></div><div className="centers-table">{centros.map((center) => <button className="center-row" onClick={() => openDetail({ id: center.id, numero: center.nome, tipo: 'Centro de distribuição', centroId: center.id, orgao: center.responsavel, emissao: '2025-01-01', vencimento: '2026-01-01', status: center.status as Status, criticidade: 'Baixa', observacao: `${center.cidade} — ${center.estado}` })} key={center.id} data-testid={`button-center-${center.id}`}><span className="center-symbol">{center.nome.split(' ').map((w) => w[0]).join('').slice(0, 2)}</span><span><b>{center.nome}</b><small>{center.cidade} — {center.estado}</small></span><span className="center-owner">{center.responsavel}</span><StatusPill status={center.status} /><ChevronRight size={15} /></button>)}</div></div><div className="activity-card"><div className="card-heading"><div><p className="eyebrow">Registro</p><h2>Atividade recente</h2></div><button className="icon-btn" onClick={() => setNotice(true)} aria-label="ver atividade recente"><MoreHorizontal size={18} /></button></div><div className="activity-list"><div><span className="activity-avatar avatar-olive">BL</span><p><b>Bianca Lima</b> atualizou o <strong>PGRS — CD Extrema</strong><small>Há 32 min · Documentos</small></p></div><div><span className="activity-avatar avatar-sand">RN</span><p><b>Ricardo Nunes</b> concluiu uma tarefa<small>Há 2 h · CD Jundiaí</small></p></div><div><span className="activity-avatar avatar-blue">HS</span><p><b>Helena Souza</b> adicionou uma evidência<small>Ontem · Licença SEMAD-GO</small></p></div></div></div></div></main>;
}
...
function ListHeader({ count, label, onAdd, addLabel }: { count: number; label: string; onAdd?: () => void; addLabel?: string }) {
  return <div className="list-header"><span><b>{count}</b> {label}</span>{onAdd && addLabel && <button className="secondary-btn" onClick={onAdd} data-testid="button-list-add"><Plus size={16} />{addLabel}</button>}</div>;
}
...
function CondicionantesPage({ items, setItems, openDetail }: { items: Condicionante[]; setItems: (v: Condicionante[] | ((old: Condicionante[]) => Condicionante[])) => void; openDetail: (item: Condicionante) => void }) {
  ...
  return <main className="content"><PageHeader eyebrow="Rotina de conformidade" title="Condicionantes" description="Prazos e responsáveis sempre visíveis." action={add} actionLabel="Nova condicionante" /><div className="list-card animate-rise"><FilterBar search={search} setSearch={setSearch} filter={filter} setFilter={setFilter} options={['Pendente', 'Em análise', 'Concluída']} placeholder="Buscar por obrigação ou responsável" onClear={() => { setSearch(''); setFilter('Todos'); }} /><ListHeader count={filtered.length} label="obrigações monitoradas" onAdd={add} addLabel="Adicionar obrigação" /><div className="condition-list">{filtered.map((item) => <div className="condition-row" key={item.id} data-testid={`row-condicionante-${item.id}`}><button className={cn('check-box', item.status === 'Concluída' && 'check-box-done')} onClick={() => toggle(item)} aria-label={`marcar ${item.titulo}`}>{item.status === 'Concluída' && <Check size={14} />}</button><button className="condition-main" onClick={() => openDetail(item)}><span><b className={item.status === 'Concluída' ? 'condition-done' : ''}>{item.titulo}</b><small>{centerName(item.centroId)} · {item.recorrencia}</small></span><span className="condition-responsible"><span className="initial-avatar">{item.responsavel.split(' ').map((p) => p[0]).join('').slice(0, 2)}</span>{item.responsavel}</span><span className={cn('condition-deadline', item.status === 'Pendente' && 'deadline-soon')}><CalendarDays size={14} />{formatDate(item.prazo)}</span><StatusPill status={item.status} /><ChevronRight size={16} /></button></div>)}</div>{filtered.length === 0 && <EmptyState title="Tudo limpo por aqui" description="Nenhuma condicionante encontrada." onClear={() => { setSearch(''); setFilter('Todos'); }} />}</div></main>;
}
function UnidadesPage({ openDetail }: { openDetail: (item: Licenca) => void }) {
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('Todos');
  const filtered = centros.filter((center) => `${center.nome} ${center.cidade} ${center.estado} ${center.responsavel}`.toLowerCase().includes(search.toLowerCase()) && (filter === 'Todos' || center.status === filter));
  const openCenter = (center: Centro) => openDetail({ id: center.id, numero: center.nome, tipo: 'Centro de distribuição', centroId: center.id, orgao: center.responsavel, emissao: '2025-01-01', vencimento: '2026-01-01', status: center.status as Status, criticidade: 'Baixa', observacao: `${center.cidade} — ${center.estado}` });
  return <main className="content"><PageHeader eyebrow="Rede operacional" title="Unidades" description="Centros acompanhados em um só lugar." /><div className="list-card units-list-card animate-rise"><FilterBar search={search} setSearch={setSearch} filter={filter} setFilter={setFilter} options={['Operando', 'Em expansão']} placeholder="Buscar unidade ou cidade" onClear={() => { setSearch(''); setFilter('Todos'); }} /><ListHeader count={filtered.length} label="unidades acompanhadas" /><div className="unit-list">{filtered.map((center) => <button className="unit-row" onClick={() => openCenter(center)} key={center.id} data-testid={`row-unidade-${center.id}`}><span className="unit-symbol">{center.nome.split(' ').map((word) => word[0]).join('').slice(0, 2)}</span><span className="unit-name"><b>{center.nome}</b><small>{center.cidade} — {center.estado}</small></span><span className="unit-owner">{center.responsavel}</span><StatusPill status={center.status} /><ChevronRight size={16} /></button>)}</div>{filtered.length === 0 && <EmptyState title="Nenhuma unidade encontrada" description="Ajuste a busca ou o filtro." onClear={() => { setSearch(''); setFilter('Todos'); }} />}</div></main>;
}
function TarefasPage({ items, setItems, openDetail }: { items: Condicionante[]; setItems: (v: Condicionante[] | ((old: Condicionante[]) => Condicionante[])) => void; openDetail: (item: Condicionante) => void }) {
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('Todos');
  const filtered = items.filter((item) => `${item.titulo} ${item.responsavel} ${centerName(item.centroId)}`.toLowerCase().includes(search.toLowerCase()) && (filter === 'Todos' || item.status === filter));
  const toggle = (item: Condicionante) => setItems((old) => old.map((current) => current.id === item.id ? { ...current, status: current.status === 'Concluída' ? 'Pendente' : 'Concluída' } : current));
  const add = () => setItems((old) => [{ id: `con-${Date.now()}`, titulo: 'Nova tarefa ambiental', licencaId: 'lic-1', centroId: 'cd-sp', responsavel: 'Marina Azevedo', prazo: '2025-06-30', status: 'Pendente', recorrencia: 'Anual' }, ...old]);
  return <main className="content"><PageHeader eyebrow="Rotina de conformidade" title="Tarefas" description="Prazos, responsáveis e evidências em sequência." action={add} actionLabel="Nova tarefa" /><div className="list-card tasks-list-card animate-rise"><FilterBar search={search} setSearch={setSearch} filter={filter} setFilter={setFilter} options={['Pendente', 'Em análise', 'Concluída']} placeholder="Buscar tarefa ou responsável" onClear={() => { setSearch(''); setFilter('Todos'); }} /><ListHeader count={filtered.length} label="tarefas monitoradas" onAdd={add} addLabel="Adicionar tarefa" /><div className="condition-list">{filtered.map((item) => <div className="condition-row" key={item.id} data-testid={`row-tarefa-${item.id}`}><button className={cn('check-box', item.status === 'Concluída' && 'check-box-done')} onClick={() => toggle(item)} aria-label={`concluir ${item.titulo}`}>{item.status === 'Concluída' && <Check size={14} />}</button><button className="condition-main" onClick={() => openDetail(item)}><span><b className={item.status === 'Concluída' ? 'condition-done' : ''}>{item.titulo}</b><small>{centerName(item.centroId)} · {item.recorrencia}</small></span><span className="condition-responsible"><span className="initial-avatar">{item.responsavel.split(' ').map((p) => p[0]).join('').slice(0, 2)}</span>{item.responsavel}</span><span className={cn('condition-deadline', item.status === 'Pendente' && 'deadline-soon')}><CalendarDays size={14} />{formatDate(item.prazo)}</span><StatusPill status={item.status} /><ChevronRight size={16} /></button></div>)}</div>{filtered.length === 0 && <EmptyState title="Nenhuma tarefa encontrada" description="Ajuste a busca ou o filtro." onClear={() => { setSearch(''); setFilter('Todos'); }} />}</div></main>;
}
function DocumentosPage({ items, setItems, openDetail }: { items: Documento[]; setItems: (v: Documento[] | ((old: Documento[]) => Documento[])) => void; openDetail: (item: Documento) => void }) {
  ...
  return <main className="content"><PageHeader eyebrow="Acervo" title="Documentos" description="Evidências organizadas por unidade e validade." action={add} actionLabel="Adicionar documento" /><div className="list-card documents-list-card animate-rise"><FilterBar search={search} setSearch={setSearch} filter={filter} setFilter={setFilter} options={['Licenças', 'Relatórios', 'Planos e programas', 'Autorizações', 'Vigente', 'A vencer', 'Expirada']} placeholder="Buscar documento, unidade ou categoria" onClear={() => { setSearch(''); setFilter('Todos'); }} /><ListHeader count={filtered.length} label="documentos encontrados" onAdd={add} addLabel="Adicionar documento" /><div className="document-grid">{filtered.map((item) => <button className="document-card" key={item.id} onClick={() => openDetail(item)} data-testid={`card-documento-${item.id}`}><span className="document-icon"><FileText size={19} /></span><span className="document-content"><b>{item.nome}</b><small>{centerName(item.centroId)} · {item.categoria}</small><span className="document-meta"><span>Validade {item.validade}</span><span>{item.tamanho}</span></span></span><span className="document-status"><StatusPill status={item.status} /><MoreHorizontal size={17} /></span></button>)}</div>{filtered.length === 0 && <EmptyState title="Nenhum documento encontrado" description="Ajuste a busca ou o filtro." onClear={() => { setSearch(''); setFilter('Todos'); }} />}</div></main>;
}
function EmptyState({ title, description, onClear }: { title: string; description: string; onClear: () => void }) { return <div className="empty-state"><div className="empty-mark"><Mark /></div><h3>{title}</h3><p>{description}</p><button className="quiet-btn" onClick={onClear}>Limpar filtros</button></div>; }
...
function Login() {
  ...
  return <div className="login-page">
    ...
    <footer className="login-footer"><span>© 2025 Licere</span><span>Dados locais de demonstração</span><div><button type="button" onClick={() => setFeedback('Termos disponíveis na versão corporativa da Licere.')} data-testid="button-login-terms">Termos de uso</button><button type="button" onClick={() => setFeedback('Política de privacidade disponível na versão corporativa da Licere.')} data-testid="button-login-privacy">Privacidade</button></div></footer>
  </div>;
}
function Shell() {
